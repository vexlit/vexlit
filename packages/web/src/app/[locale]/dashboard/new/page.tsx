...
export default function NewScanPage() {
    ...
    async function loadRepos() {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          setReposError(data.errorCode ? t(data.errorCode) : (data.error ?? t("failedToLoadRepos")));
          return;
        }
        const data = await res.json();
        setRepos(data);
      } catch {
        setReposError(t("failedToConnectGithub"));
      } finally {
  ...
  const handleGitHubScan = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setError(data.errorCode ? t(data.errorCode) : (data.error ?? t("scanFailed")));
        return;
      }
        ...
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-300 mb-2">
              {t("githubRepoUrl")}
              <span className="text-gray-500 font-normal"> {t("anyPublicRepo")}</span>
            </label>
            <input
              type="text"
          ...
          <div className="flex items-center gap-4">
            ...
            <span className="text-gray-500 text-sm">{t("orSelectRepo")}</span>
            <div className="flex-1 border-t border-gray-800" />
          </div>
            ...
            <div className="max-h-64 overflow-y-auto border border-gray-800 rounded-lg divide-y divide-gray-800">
              {filteredRepos.length === 0 && (
                <p className="text-gray-500 text-sm p-4">
                  {repoSearch ? t("noMatchingRepos") : t("noReposFound")}
                </p>
              )}
          ...
          >
            {loading ? t("fetchingScanning") : t("startScan")}
          </button>
        </div>
      )}
